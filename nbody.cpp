/*   Developed By: Sanjaya Pandey
 *   Code base from: Gravity Lab   
 *  O  `//\                 /|\                                               |
 *  |     /\                                                                  |
 *=============================================================================
 *
...
int main(int argc, char *argv[])
{
    ...
    real  dt_tot = 1000;         // duration of the integration
    ...
    std::cout << "Time difference = " << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() <<" milli second"<<std::endl;
}
                           ...
                           real & coll_time)
{
    ...
    real coll_est_q[n];                           // collision time scale estimate

    int i;
    //#pragma omp parallel for reduction(-:epot) shared(acc, jerk, coll_time_q) schedule(static)         // to 4th power (quartic)
    for ( i = 0; i < n ; i++){
	real tempAcc[n][NDIM];
	real tempJerk[n][NDIM];
	real tempAcc1[n][NDIM];
	real tempJerk1[n][NDIM];
	...
	omp_set_num_threads(0); // Use 4 threads for all consecutive parallel regions
	#endif
	#pragma omp parallel
	#pragma omp for reduction(-:epot) private(rji,vji,da,dj) schedule(static)
...
// add the {j (i)} contribution to the {i (j)} values of acceleration and jerk:
		...
		*/
            ...
            for (int k = 0; k < NDIM ; k++){
		tempAcc[j][k] = mass[j] * da[k];                 // using symmetry
		//tempAcc1[j][k] -= mass[i] * da[k];                 // find pairwise

                //acc[i][k] += mass[j] * da[k];                 // using symmetry
                acc[j][k] -= mass[i] * da[k];                 // find pairwise

		tempJerk[j][k] = mass[j] * dj[k];                // acceleration
		//tempJerk1[j][k] -= mass[i] * dj[k];                // and jerk
...
// second collision time estimate, based on free fall:
	    ...
	    */
	//updated code
	    if(coll_est_q[j]> (r2/da2)) coll_est_q[j] = r2/da2;
        } 
	for (int j = i+1; j < n ; j++){ 
		for (int k = 0; k < NDIM ; k++){
			acc[i][k] += tempAcc[j][k];
			jerk[i][k] += tempJerk[j][k];

		}
	}                                    
    }   
     for ( i = 1; i < n ; i++){
	if (coll_time_q> coll_est_q[i])
                coll_time_q = coll_est_q[i];
	}
